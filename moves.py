...
def print_move(piece, location, prom, attacking, old_location, check):
    d = {'rook': 'R', 'bishop': 'B', 'knight': 'N', 'queen': 'Q', 'king': 'K', 'pawn': ''}
    if prom[0]:
        promotion = f'={prom[1]}'
    else:
        promotion = ''
...
class MoveMaker:
    def __init__(self, move_list, game_tiles, game_pieces, player, ep, checks, root, move_number, print_move_on):
    ...
    def first_tile_pressed(self, location, current_tile):
        ...
        if location in self.game_pieces.keys() and self.game_pieces[location].piece_colour == self.player.get():
    ...
    def current_player_check(self):
        ...
        if this_players_king_position in get_attacked_positions(self.game_pieces, self.enemy_colour):
            get_sound(3)
            res = self.checks[defender_colour] = self.game_pieces[this_players_king_position].in_check = True
        else:
            res = self.checks[defender_colour] = self.game_pieces[this_players_king_position].in_check = False
        return res
    ...
    def enemy_player_check(self):
        ...
        if enemy_king_position in get_attacked_positions(self.game_pieces, player_colour):
